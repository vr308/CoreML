...
# Sampling from a DP using a stick breaking view
...
#Understanding the impact of the alpha parameter 

alpha = 10
n = np.linspace(1,100, 100)
y = []
for i in n:
    y.append(alpha/(alpha + i-1))
    

plt.figure()
plt.plot(n,y)

# Dirichlet process location mixture of Gaussians
...
alpha = 2
P0_loc = sp.stats.norm
P0_var = sp.stats.invgamma
f = lambda x, theta, sigma: sp.stats.norm.pdf(x, theta, np.square(sigma))
...
theta = P0_loc.rvs(size=(N, K))
sigma = P0_var.rvs(3,0.5,size=(N,K))

x_plot = np.linspace(-5, 5, 200)

dpm_pdf_components = f(x_plot[np.newaxis, np.newaxis, :], theta[..., np.newaxis], sigma[...,np.newaxis])
